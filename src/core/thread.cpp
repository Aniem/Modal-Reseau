...
namespace Modal
{
    ...
    bool Thread::start() {
        ...
        MODAL_ASSERT(thread_create_status == 0);
        MODAL_ASSERT(this->thread != 0);
        return true;
    }
    ...
    void Thread::wait() {
        ...
        MODAL_ASSERT(thread_join_status == 0);
    }
    ...
    void Thread::term() {
        if (this->thread != 0) {
            ...
            MODAL_ASSERT(thread_cancel_status == 0 || thread_cancel_status == ESRCH);
            this->thread = 0;
        }
    }
    ...
    void* Thread::_thread_start(void *arg) {
        try {
            // Call Thread::run()
            Modal::Thread *self = (Modal::Thread*) arg;
            MODAL_ASSERT(self != NULL);
